...
from keras.layers import Dense,Conv2D,MaxPooling2D,ZeroPadding2D,AveragePooling2D,TimeDistributed,Add,GlobalAveragePooling2D
...
def channel_attention_shrinkage_block(input_tensor, kernel_size, filters, stage, block):
    filters1, filters2, filters3 = filters
    ...
    scales = Dense(filters3, activation=None, kernel_initializer='he_normal', 
                   kernel_regularizer=l2(1e-4))(abs_mean)
    scales = BatchNormalization()(scales)
    scales = Activation('relu')(scales)
    scales = Dense(filters3, activation='sigmoid', kernel_regularizer=l2(1e-4))(scales)
    ...
    x = keras.layers.multiply([Lambda(sign_backend)(x), n_sub])
...
def CASNet(inputs):
    ...
    x = channel_attention_shrinkage_block(x, 3, [256, 256, 1024], stage=4, block='b')
    x = channel_attention_shrinkage_block(x, 3, [256, 256, 1024], stage=4, block='c')
    x = channel_attention_shrinkage_block(x, 3, [256, 256, 1024], stage=4, block='d')
    x = channel_attention_shrinkage_block(x, 3, [256, 256, 1024], stage=4, block='e')
    x = channel_attention_shrinkage_block(x, 3, [256, 256, 1024], stage=4, block='f')
